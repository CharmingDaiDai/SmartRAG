...
# 2. 复制并编辑环境变量配置文件
cp .env.example .env
# 编辑 .env，填入你的配置（至少需要：GLM_API_KEY、EMBEDDING_BASE_URL 等）
...
| Attu 向量可视化 | http://localhost:8000 | Milvus 数据管理面板 |
...
所有运行时配置通过项目根目录的 `.env` 文件注入（Docker Compose 自动读取）。首次部署请执行：
...
```dotenv
MYSQL_ROOT_PASSWORD=your_strong_root_password   # MySQL root 密码
MYSQL_PASSWORD=your_strong_user_password        # MySQL 用户密码
DB_PASSWORD=your_strong_user_password           # 必须与 MYSQL_PASSWORD 一致
JWT_SECRET=your_strong_random_secret            # JWT 签名密钥（openssl rand -hex 32）
...
`docker-compose.yml` 同时支持构建与运行，一条命令即可完成部署：
