...
(
    ...
    `index_status` VARCHAR(50)  NOT NULL DEFAULT 'UPLOADED' COMMENT '索引状态: UPLOADED, READING, PARSING, CHUNKING, TREE_BUILDING, LLM_ENRICHING, SAVING, EMBEDDING, STORING, INDEXED, ERROR',
...
(
    ...
    `key_knowledge` TEXT         DEFAULT NULL COMMENT '语义压缩提取的核心知识点(JSON数组，HiSem-Fast使用)',
    `summary`       TEXT         DEFAULT NULL COMMENT '语义压缩提取的内容摘要(HiSem-Fast使用)',
    `strategy_type` VARCHAR(50)  DEFAULT NULL COMMENT '索引策略类型: NAIVE_RAG/HISEM_RAG_FAST/HISEM_RAG',
    `created_at`  DATETIME     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`  DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
(
    ...
    `title_path`        VARCHAR(1000)        NOT NULL COMMENT '多级标题路径（如：第一章 -> 1.1节 -> 1.1.1小节）',
...
(
    ...
    `current_step`     VARCHAR(50)           DEFAULT NULL COMMENT '当前步骤: READING, PARSING, CHUNKING, TREE_BUILDING, LLM_ENRICHING, SAVING, EMBEDDING, STORING',
